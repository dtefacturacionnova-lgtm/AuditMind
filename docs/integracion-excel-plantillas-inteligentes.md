...
> **Estado: FASE 0 COMPLETA EN CÓDIGO, PENDIENTE DE DEPLOY.** Motor genérico (EXC-01/02), endpoints REST (EXC-03) y componente de UI (EXC-04) construidos, comiteados y pusheados a `main`, verificados en dos niveles — prueba de humo a nivel de servicio (17/17) y prueba de extremo a extremo vía HTTP real con login demo (9/9), EXC-05. **El deploy a `vps-muestreo` quedó pendiente**: el VPS no fue alcanzable por SSH (timeout de conexión) al intentar el pull/build/restart — reintentar en la próxima sesión. El registro de plantillas sigue vacío: hoy no hay ninguna de las 6 plantillas del catálogo (fases 1-5) publicada. Este documento existe para no perder el diseño entre sesiones; actualizar la sección 7 (bitácora) cada vez que se retome.
...
- **Deploy: PENDIENTE.** `git push` a `main` sí se completó (commits EXC-01..EXC-05 en GitHub). El intento de desplegar a `vps-muestreo` (`ssh vps-muestreo "... git pull && turbo build ..."`) falló por **timeout de conexión SSH al puerto 22** — el VPS no fue alcanzable desde el entorno de esta sesión. No se tocó nada en el servidor; sigue corriendo el build anterior a esta fase. Reintentar el deploy en la próxima sesión con el patrón de deploy estándar del proyecto.

**Fase 0 completa en código, no en producción.** El motor genérico, los endpoints y el componente de UI están construidos y verificados en dos niveles (servicio y HTTP real), pero el deploy queda como primer pendiente de la próxima sesión — antes de eso, ningún cambio de esta fase es visible en `https://auditoria.ianovatechsystems.com`. Después del deploy, lo único que falta para que un auditor pueda usar esto de verdad es construir la primera plantilla real (§6, fase 1: Composición de Cuenta).
