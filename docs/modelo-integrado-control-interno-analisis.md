...
**Mecanismo técnico** (para cuando se decida implementar): un endpoint de solo lectura tipo `GET /audits/:id/risks/:riskRef/trace`, que agrega — sin persistir nada nuevo — las filas relevantes de los papeles de la cadena que ya existen en esa auditoría. Mismo principio que `paper-graph.service.ts` ya aplica a nivel de papel, aplicado ahora a nivel de fila/riesgo.
...
| 4a | Fase 6a (§8.2, §8.4) | Diseño + endpoint `GET /audits/:id/risks/:riskRef/trace` (agregación de solo lectura entre 6-8 papeles) | Fases 0-1 | Medio | **Fable/Opus** | Arquitectura nueva de agregación cross-papel — el diagnóstico correcto de qué leer de cada papel y cómo estructurar la respuesta es lo caro de equivocar aquí |
| 4b | Fase 6b (§8.2-8.3, §8.5-8.8) | Construcción del cockpit: pestaña "Control Interno", stepper, Ficha de Riesgo (drawer), dashboard ejecutivo | Fase 6a | Medio (UI nueva) | **Sonnet** | Con la forma de datos del endpoint ya definida (4a), es UI siguiendo patrones ya usados en Cartera (stepper) y `PaperLink` (navegación) |
| 5 | Fase 2 (§6, §8.9) | `lane?` + `kind:"control"` en el nodo del flujograma; badge de color por `rowId`→MRCI | Fase 0.5 | Bajo-Medio | **Sonnet** | Extensión aditiva de un modelo de datos ya entendido, mismo patrón que `linkedPaper` en PBC/PT-HALL-COM |
| 6 | Fase 3 (§6) | Sección de segregación de funciones en `PT-A3` → alimenta `PT-MRCI` | Fase 0 | Bajo | **Sonnet** | Mismo patrón `MATRIX`+`aiHint` que el resto del papel |
| 7 | Fase 4 (§6) | Reglas de lógica de auditoría en el Quality Gate, solo para `PT-A5`/`PT-MRCI` | Fase 0 | Bajo | **Sonnet** | Reglas ya especificadas en el diseño (riesgo sin control, control sin riesgo, residual alto sin plan) — implementación directa, no diseño nuevo |
| 8 | Fase 5 (§6) | `aiHint` de conclusión diferenciado por perfil en `PT-MRCI` S4 | — | ~0 (texto) | **Haiku** | Cambio de texto puro, sin lógica |
| 9 | Fase 7 (§6, §8.9) | Renderer de PDF del Reporte Integrado (flujograma+tabla+mapa de calor+conclusión+recomendaciones) | Fases 2, 0.5 | Medio | **Sonnet** | Sigue el patrón ya probado de `coso-pdf.ts`/`nia530-pdf.ts` — requiere que la decisión de mapa de calor (abajo) ya esté tomada antes de empezar |

**Decisión pendiente del usuario antes de la Fase 7**: mapa de calor Probabilidad×Impacto (requiere separar `PT-A2` en 2 ejes) vs. reutilizar el mapa de calor Área×Nivel ya existente en `PT-MRCI` S3 (sin tocar `PT-A2`) — tabla comparativa en §8.9. No bloquea las Fases 0-6, se puede decidir más adelante.
