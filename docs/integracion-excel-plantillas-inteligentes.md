...
> **Estado (2026-08-16): CATÁLOGO COMPLETO — LAS 5 PLANTILLAS EN PRODUCCIÓN.** Motor genérico (EXC-01/02), endpoints REST (EXC-03) y componente de UI (EXC-04), y las 5 plantillas del catálogo — Composición de Cuenta (§3.6), Conciliación Bancaria (§3.7), Revisión Analítica NIA 520 (§3.8), Circularización de CxC NIA 505 (§3.9), Arqueo de Caja NIA 501 (§3.10) — están construidas, verificadas con datos demo reales, desplegadas y confirmadas en producción (deploy batched de fases 2-5, ver §3.11). El encargo "Empresa Comercial Demo SA de CV" tiene además un ejemplo real de cada plantilla dejado en pantalla (§3.11), no solo en pruebas revertidas. Este documento existe para no perder el diseño entre sesiones; actualizar la sección 7 (bitácora) cada vez que se retome.
...
## 3.11 Deploy de fases 2-5 y ejemplos reales en el encargo principal (2026-08-16)

**Deploy completo**: en cuanto el usuario confirmó datos móviles disponibles, se desplegaron las 4 fases pendientes (commits hasta `dc0aa31`) — `git pull` (fast-forward, 10 archivos), build 2/2 exitoso (1m35s), `pm2 restart auditmind-api auditmind-web`. Verificado: arranque limpio sin errores nuevos, las 2 rutas del motor de plantillas Excel mapeadas, `.next` recién compilado (timestamp posterior al build, no una copia vieja), login público 200, endpoint del motor 401 sin token. **Las 5 fases están en producción.**

A pedido del usuario, se llenó además UN ejemplo real por cada plantilla en "Empresa Comercial Demo SA de CV" (`cmsrskxz80001jjekqqy3t4xy`) — el encargo usado durante toda la sesión (COSO, PT-MRCI, PT-NIA530, etc.) — vía el ciclo real de la plantilla (`generar()` → editar celdas como lo haría un auditor → `leer()`), **sin revertir** (a diferencia de las pruebas de humo de cada fase). Estado dejado:

| Plantilla | Papel | Qué quedó documentado |
|---|---|---|
| Composición de Cuenta | C-04 — Activo Fijo PP&E (`cmsrsl0yd0086jjekjym73uk4`), S7 | 3 cuentas de mobiliario/equipo/vehículo, suma $420,000 (coincide con la cuenta 1501 del TB) — incluye un ajuste propuesto de $5,000 por valor residual no considerado. |
| Conciliación Bancaria | C-01 — Caja y Bancos (`cmsrsl0n2007gjjekrnmrieor`), S1 | Diferencia de -$2,000 (cheque pendiente de cobro $1,200 + comisión bancaria no registrada $700), marcada `_excelOrigen: 'CONCILIACION_BANCARIA'`. |
| Arqueo de Caja | mismo papel C-01, S1 | Diferencia de -$200 (conteo físico $13,800 + vale de caja chica $1,000 vs. $15,000 según libros), marcada `_excelOrigen: 'ARQUEO_CAJA'` — **coexiste** en la misma S1 con la fila de Conciliación Bancaria y con un hallazgo manual preexistente ($8,500, sin marcador), sin que ninguna se pise entre sí. |
| Revisión Analítica | B-07 — Análisis de Variaciones (`cmsrsl0bt006mjjek4cp8mp7d`), S1c | Las 3 únicas variaciones que superan el filtro real de esta auditoría (>20% y > ME=$30,000): Préstamos Bancarios CP (-25%, explicada y razonable), Utilidades Retenidas Inicio Año (-116%, requiere procedimiento adicional — cifra grande a propósito, para mostrar que el filtro no exige que la explicación sea siempre "Sí"), Gastos Financieros (+38%, explicada y razonable). |
| Circularización de CxC | PT-NIA530 (`cmst947bg000nbhahi4du8brm`), S5 | 2 de los 18 ítems ya existentes documentados con confirmación positiva recibida (sin tocar los montos, que ya coincidían), 1 documentado con procedimiento alternativo por falta de respuesta (con referencia cruzada a C-01) — demuestra ambos caminos que la plantilla soporta. |

Los valores en dólares se diseñaron para tener sentido dentro del expediente ya existente (p. ej. la suma de Composición de Cuenta coincide con el TB; la explicación de Gastos Financieros se apoya en el aumento real de Préstamos Bancarios LP que ya estaba en B-07 S1). 20/20 verificaciones pasaron durante el llenado (cada plantilla comprobó que su propia fila/sección quedó bien formada y que no tocó nada ajeno).
...
### 5.5.2 Implementación (cerrada el 2026-08-16)

Construida sobre `EXCEL_MISMO_PAPEL`/`FUSIONA` ya existentes, sin tocar ninguna de las 5 plantillas de negocio. Piezas nuevas:

- **`excel-generic-template.service.ts`**: `construirDefGenerica(paperCode, entradas)` — infiere columnas de MATRIX desde las filas YA existentes (mismo principio que `MatrixGridPanel.deriveColumns()`) y, si la sección está vacía, hace *bootstrap* parseando el prefijo `"Columnas: A | B | C"` que casi todos los `aiHint` del sistema ya siguen. Si ninguna de las dos fuentes da nada usable, la sección se OMITE con motivo explícito (nunca una tabla sin encabezados). `construirDefDesdeLayout(layout)` reconstruye el mismo `def` en el camino de vuelta, desde un descriptor JSON sellado en el manifiesto — no se vuelve a inferir de datos que pudieron cambiar mientras el auditor trabajaba fuera de línea.
- **Manifiesto**: nuevo campo `genericLayout` (mismo patrón que `areaKey` — sellado en la firma HMAC, restaurado al importar). Se agregó a `payload()`/`verificarManifest()` a la vez, en el mismo cambio, para no repetir el bug que sí ocurrió con `areaKey` en la revisión de calidad de EXC-QA.
- **Motor**: dos métodos nuevos en `ExcelTemplateEngineService` — `generarGenerica()` y `leerGenerica()` — que construyen/reconstruyen el `def` y delegan el resto (escritura, lectura, protección de hojas, verificación de firma) a los mismos `generar()`/`leer()` que usan las 6 plantillas de negocio. Cero duplicación del motor real.
- **Endpoints**: `GET :id/excel-generic?sections=S1,S2,...` y `POST :id/excel-generic/import`.
- **UI**: `WorkOfflinePanel.tsx`, montado UNA vez por papel (no por sección) en `SmartPaperSections.tsx`, justo debajo de la barra de completitud — checklist de las secciones elegibles, descarga, subida, y aviso de qué secciones quedaron fuera y por qué.
- **Bug real encontrado y corregido durante la prueba**: `filasMinimas` inicial era `Math.max(filas.length, 5)` — con una sección que YA tenía ≥5 filas, esto dejaba el rango con nombre fijado exactamente al número de filas existentes, sin ninguna fila en blanco para que el auditor agregara un ítem nuevo fuera de línea. Corregido a `filas.length + 10` siempre. Encontrado por una prueba real (no una revisión de código) que intentaba agregar una fila y fallaba exactamente por esto.
- **Verificación**: prueba de humo contra el papel `PT-FIN-C-SUST` real de C-04 (Activo Fijo) — 12/12 OK: generación con una clave inexistente mezclada (queda en `omitidas`, no rompe el resto), ida/vuelta de MATRIX (S2, con fila nueva agregada Y filas originales conservadas) y de TEXTAREA (S5) a la vez, manifiesto alterado a mano se rechaza. Verificado también en el navegador real (Browser pane): el panel aparece en el lugar correcto, el checklist muestra las 6 secciones elegibles del papel, la descarga real vía UI responde 200 OK sin errores de consola.
- **Deploy: PENDIENTE** — confirmar con el usuario antes de subir al VPS (es trabajo nuevo, no parte del lote de fases 2-5 ya autorizado).
...
| 2026-08-16 | **Deploy de fases 2-5 + ejemplos reales** (§3.11): usuario confirmó datos móviles → `git pull` + build 2/2 (1m35s) + `pm2 restart`, verificado limpio en producción (rutas mapeadas, `.next` fresco, 401 sin token, login 200). Se llenó además un ejemplo real por plantilla en el encargo "Empresa Comercial Demo SA de CV" (el usado durante toda la sesión), sin revertir esta vez — ver tabla en §3.11 para el detalle de qué quedó en cada papel. | **Las 5 fases están en producción y el encargo principal tiene un ejemplo real de cada una.** 20/20 verificaciones durante el llenado. Catálogo de EXC-01 a EXC-23 cerrado por completo. |
| 2026-08-16 | **Plantilla Excel genérica — "trabajar fuera de línea"** (§5.5, EXC-24..30): layout de `ExcelTemplateDef` generado en tiempo de ejecución desde `PAPER_TEMPLATES` (sin `.template.ts` por papel) — cubre MATRIX (columnas inferidas de filas existentes o de `aiHint`) y los 8 tipos escalares simples, ~93% de las 526 secciones del sistema. `generarGenerica()`/`leerGenerica()` nuevos en el motor, delegan todo lo demás a `generar()`/`leer()` ya existentes. Endpoints `GET/POST :id/excel-generic`. UI: `WorkOfflinePanel.tsx`, un panel por papel con checklist de secciones elegibles. | **Implementado y verificado**: 12/12 en la prueba de humo contra C-04 real (MATRIX + TEXTAREA a la vez, manifiesto alterado rechazado) + verificado en el navegador real (panel visible, descarga 200 OK, sin errores de consola). Un bug real encontrado y corregido: `filasMinimas` no dejaba espacio para filas nuevas cuando la sección ya tenía ≥5 filas. Pendiente: deploy (confirmar con el usuario, es trabajo nuevo fuera del lote ya autorizado). |
