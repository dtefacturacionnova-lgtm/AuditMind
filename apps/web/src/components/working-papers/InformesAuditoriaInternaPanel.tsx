...
interface Props {
  paperId:  string;
  rows:     InformeAIRow[];
  onChange: (rows: InformeAIRow[]) => void;
  readOnly?: boolean;
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

function newRow(): InformeAIRow {
  return {
    id:            `infai_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 6)}`,
    referencia:    '',
    planTrabajo:   '',
    nombreInforme: '',
    alcance:       '',
...
interface RowProps {
  ...
  paperId:  string;
  readOnly: boolean;
  onUpdate: (patch: Partial<InformeAIRow>) => void;
  onDelete: () => void;
}

function InformeRowItem({ row, index, paperId, readOnly, onUpdate, onDelete }: RowProps) {
  ...
  return (
    <tr className="group border-b border-gray-50 last:border-0 hover:bg-blue-50/20 transition-colors">
      ...
      <td className={`${cellCls} min-w-[110px]`}>
        <input
          type="text"
          value={row.planTrabajo}
          onChange={e => onUpdate({ planTrabajo: e.target.value })}
          disabled={readOnly}
          placeholder="Plan AI 2025"
          className={inputCls}
        />
      </td>
      ...
      <td className={`${cellCls} min-w-[160px]`}>
        <input
          type="text"
          value={row.nombreInforme}
          onChange={e => onUpdate({ nombreInforme: e.target.value })}
          disabled={readOnly}
          placeholder="Evaluación de Cuentas por Cobrar"
          className={inputCls}
        />
      </td>
      ...
      <td className={`${cellCls} min-w-[140px]`}>
        <textarea
          value={row.alcance}
          onChange={e => onUpdate({ alcance: e.target.value })}
          disabled={readOnly}
          rows={2}
          placeholder="Área / período cubierto…"
      ...
      <td className={`${cellCls} w-32`}>
        <input
          ...
          onChange={e => onUpdate({ fechaEmision: e.target.value })}
          disabled={readOnly}
        ...
        {readOnly ? (
          row.conHallazgos === 'SI' ? (
            <span className="inline-flex items-center gap-1 text-xs text-amber-700 font-medium">
              <AlertCircle className="w-3.5 h-3.5" /> Sí
            </span>
          ) : row.conHallazgos === 'NO' ? (
          ...
          <div className="flex gap-1">
            <button
              type="button"
              onClick={() => onUpdate({ conHallazgos: 'SI' })}
              className={`px-2 py-0.5 rounded text-[11px] font-medium border transition-colors ${
                row.conHallazgos === 'SI'
                  ? 'bg-amber-500 text-white border-amber-500'
                  : 'text-gray-500 border-gray-200 hover:border-amber-300 hover:text-amber-700'
              }`}
            >
              Sí
            </button>
            <button
              type="button"
              onClick={() => onUpdate({ conHallazgos: 'NO' })}
              className={`px-2 py-0.5 rounded text-[11px] font-medium border transition-colors ${
                row.conHallazgos === 'NO'
      ...
      <td className={`${cellCls} w-28`}>
        {readOnly ? (
          ...
          <div className="flex flex-col gap-1">
            <button
              type="button"
              onClick={() => onUpdate({ estado: 'ATENDIDO' })}
              className={`px-2 py-0.5 rounded text-[11px] font-medium border transition-colors ${
                row.estado === 'ATENDIDO'
                  ? 'bg-emerald-600 text-white border-emerald-600'
                  : 'text-gray-500 border-gray-200 hover:border-emerald-300 hover:text-emerald-700'
...
export function InformesAuditoriaInternaPanel({ paperId, rows, onChange, readOnly = false }: Props) {
  function addRow() {
    onChange([...rows, newRow()]);
  }

  function updateRow(id: string, patch: Partial<InformeAIRow>) {
  ...
  const conHallazgos = rows.filter(r => r.conHallazgos === 'SI').length;
      ...
      {rows.length === 0 ? (
        <div className="py-8 text-center border border-dashed border-gray-200 rounded-xl">
          <p className="text-xs text-gray-400 mb-3">
            Sin informes registrados. Agregue los informes emitidos por Auditoría Interna en el período.
          </p>
          {!readOnly && (
            <button
              onClick={addRow}
        ...
        <div className="border border-gray-100 rounded-xl overflow-hidden">
          ...
          <div className="flex items-center gap-4 px-4 py-2 bg-gray-50 border-b border-gray-100">
            <span className="text-[11px] text-gray-500">
              {rows.length} {rows.length === 1 ? 'informe' : 'informes'}
            </span>
            {conHallazgos > 0 && (
              <span className="flex items-center gap-1 text-[11px] text-amber-700">
                <AlertCircle className="w-3 h-3" />
                {conHallazgos} con hallazgos
              </span>
            )}
            {pendientes > 0 && (
            ...
            {pendientes === 0 && rows.length > 0 && rows.every(r => r.estado === 'ATENDIDO') && (
            ...
            <table className="w-full text-sm" style={{ minWidth: '1100px' }}>
              <thead>
                <tr className="bg-white border-b border-gray-100">
                  ...
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-24">Referencia</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide min-w-[110px]">Plan de Trabajo</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide min-w-[160px]">Nombre del Informe</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide min-w-[140px]">Alcance</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-32">Fecha Emisión</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-24">Hallazgos</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-28">Estado</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-44">Informe adjunto</th>
