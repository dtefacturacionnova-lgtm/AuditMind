...
import {
  ShieldAlert, TrendingUp, DollarSign, Clock, ChevronRight,
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <h1 className="text-xl font-black">Catálogo de Riesgos del Plan</h1>
                    <p className="text-slate-400 text-sm">Paisaje de riesgo por categoría regulatoria · 8 tipos</p>
                  </div>
                </div>
              </div>
              <Link href="/dashboard/projects"
                className="flex items-center gap-1.5 text-xs text-slate-300 hover:text-white border border-slate-600 px-3 py-2 rounded-xl hover:border-slate-400 transition-colors">
                <ChevronRight className="w-3.5 h-3.5" /> Banco de Proyectos
              </Link>
            </div>
                          ...
                          return (
                            <tr key={cat.value}
                              ...
                              {levels.map(({ level, count }) => {
                                const intensity = count === 0 ? 0 : Math.max(10, Math.round((count / maxCell) * 100));
                                const cellColors: Record<string, string> = {
                                  CRITICO: `bg-red-${Math.min(9, Math.max(1, Math.round(intensity/10)))}00`,
                                  ALTO:    `bg-orange-${Math.min(9, Math.max(1, Math.round(intensity/10)))}00`,
                                  MEDIO:   `bg-amber-${Math.min(9, Math.max(1, Math.round(intensity/10)))}00`,
                                  BAJO:    `bg-green-${Math.min(9, Math.max(1, Math.round(intensity/10)))}00`,
                                };
                                return (
                  ...
                  <div className="bg-white rounded-2xl border-2 shadow-sm overflow-hidden sticky top-0"
                    ...
                    <div className="px-5 py-3 max-h-80 overflow-y-auto">
                      {sortedProjs.length === 0 ? (
                        <div className="py-6 text-center text-gray-400">
                          ...
                          <p className="text-xs mt-1">Clasifica proyectos en el Banco de Proyectos</p>
                        </div>
                      ) : (
                    ...
                    <div className="px-5 py-3 border-t border-gray-100">
                      <Link href="/dashboard/projects"
                        ...
                        Ver todos en Banco de Proyectos <ChevronRight className="w-3.5 h-3.5" />
                      </Link>
                    </div>
                  </div>
                ) : (
          ...
          {!isLoading && uncategorized > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3 text-sm text-amber-800">
              ...
              <span>
                ...
                {' '}<Link href="/dashboard/projects" className="underline hover:text-amber-900">Clasificarlos en el Banco de Proyectos →</Link>
              </span>
            </div>
          )}
